...
func HandleDeleteTransaction(
	...
	case "transaction_delete_date_from":
		...
		userState.Step = "transaction_delete_date_to"

		return tgbotapi.NewMessage(chatID, "To which date? (YYYY-MM-DD)")

	case "transaction_delete_date_to":
		_, err := time.Parse("2006-01-02", text)
		if err != nil {
			return tgbotapi.NewMessage(chatID, "Invalid date format")
		}
		...
		textMsg := "Select transaction to delete:\n\n"
		for i, tx := range transactions {
		...
		userState.Step = "transaction_delete_select"
		return tgbotapi.NewMessage(chatID, textMsg)

	case "transaction_delete_select":
		...
		userState.Step = "transaction_delete_confirm"
		msg = tgbotapi.NewMessage(chatID, "Delete this transaction?")
		msg.ReplyMarkup = keyboard.Confirm()
		return msg

	case "transaction_delete_confirm":
		...
		case "Yes":
			if err := service.DeleteTransaction(id); err != nil {
				...
				msg = tgbotapi.NewMessage(chatID, "Transaction deleted!")
			}
